...
public class AguardandoRevisaoHandler implements RoteiroStatusHandler {
    ...
    public Roteiro handle(Roteiro roteiro) {
        ...
        if (authentication != null && authentication.isAuthenticated()) {
            ...
            boolean isStatusValid = (roteiro.getStatus().getStatusId() == Status.Values.AGUARDANDO_REVISAO.getRoleId());

            if (isRoleValid && isStatusValid) {
                roteiro.setStatus(new Status(Status.Values.EM_REVISAO.getRoleId(), "EM_REVISAO"));
                return roteiro;
            }
        }
