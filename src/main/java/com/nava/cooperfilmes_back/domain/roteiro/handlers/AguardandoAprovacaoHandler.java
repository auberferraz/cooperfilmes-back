...
public class AguardandoAprovacaoHandler implements RoteiroStatusHandler {
    ...
    public Roteiro handle(Roteiro roteiro) {
        ...
        if (authentication != null && authentication.isAuthenticated()) {
            ...
            boolean isStatusValid = (roteiro.getStatus().getStatusId() == Status.Values.AGUARDANDO_APROVACAO.getRoleId());

            if (isRoleValid && isStatusValid) {
                roteiro.setStatus(new Status(Status.Values.APROVADO.getRoleId(), "APROVADO"));
                return roteiro;
            }
        }
