...
public class EmRevisaoHandler implements RoteiroStatusHandler {
    ...
    public Roteiro handle(Roteiro roteiro) {
        ...
        if (authentication != null && authentication.isAuthenticated()) {
            ...
            boolean isRoleValid = user.getRole().getName().equals("REVISOR");

            boolean isStatusValid = (roteiro.getStatus().getStatusId() == Status.Values.EM_REVISAO.getRoleId());

            if (isRoleValid && isStatusValid) {
                roteiro.setStatus(new Status(Status.Values.AGUARDANDO_APROVACAO.getRoleId(), "AGUARDANDO_APROVACAO"));
                return roteiro;
            }
        }
