...
public class EmAprovacaoHandler implements RoteiroStatusHandler {
    ...
    public Roteiro handle(Roteiro roteiro) {
        ...
        if (authentication != null && authentication.isAuthenticated()) {
            ...
            boolean isStatusValid = (roteiro.getStatus().getStatusId() == Status.Values.AGUARDANDO_ANALISE.getRoleId());

            if (isRoleValid && isStatusValid) {
                roteiro.setStatus(new Status(Status.Values.EM_ANALISE.getRoleId(), "EM_ANALISE"));
                return roteiro;
            }
        }
